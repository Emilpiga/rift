    ...
    pub fn show(self, ui: &mut Ui<'_>, rect: Rect, value: &mut String, time: f32) -> Response {
        ...
        if focused {
            ...
            if sel.has_range() {
                ...
                ui.draw_rect(
                    sel_rect,
                    theme.colors.accent.with_alpha(0.40),
                );
            }
...
fn apply_keyboard_edits(ui: &mut Ui<'_>, value: &mut String, max_chars: usize) {
    ...
    if ctrl {
        for ev in ui.input().key_events().to_vec() {
            if ev == KeyCode::KeyA {
                let len = value.len();
                ui.state_mut().text_selection = TextSelection { anchor: 0, caret: len };
            }
        }
    }
    ...
    for ev in ui.input().key_events().to_vec() {
        match ev {
            KeyCode::ArrowLeft => move_caret(ui, value, Direction::Left, ctrl, shift),
            KeyCode::ArrowRight => move_caret(ui, value, Direction::Right, ctrl, shift),
            KeyCode::Home => {
                let mut sel = ui.state().text_selection;
                sel.caret = 0;
                if !shift {
    ...
    for i in 0..dels {
        ...
        if i == 0 && sel.has_range() {
            ...
            ui.state_mut().text_selection = TextSelection { anchor: a, caret: a };
        } else {
            let sel = ui.state().text_selection;
    ...
    for i in 0..bks {
        ...
        if i == 0 && sel.has_range() {
            ...
            if sel.caret > 0 {
                ...
                ui.state_mut().text_selection = TextSelection { anchor: prev, caret: prev };
            }
        }
    }
...
fn is_shift_held(ui: &Ui<'_>) -> bool {
    ui.input().is_key_held_raw(KeyCode::ShiftLeft)
        || ui.input().is_key_held_raw(KeyCode::ShiftRight)
}

fn is_ctrl_held(ui: &Ui<'_>) -> bool {
    ui.input().is_key_held_raw(KeyCode::ControlLeft)
        || ui.input().is_key_held_raw(KeyCode::ControlRight)
}
