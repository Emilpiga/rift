...
/// RGBA8 texture (the right format for color inputs). For
/// linear data textures (normal, metallic/roughness, AO, height)
/// use [`load_texture_from_file_linear`].
pub fn load_texture_from_file<P: AsRef<std::path::Path>>(
    ...
    load_texture_from_file_with_format(
...
/// RGBA8 texture. Use this for non-color data inputs (normal
/// maps, packed metallic/roughness, AO, height) so the GPU
/// doesn't apply an sRGB → linear curve to numeric data.
pub fn load_texture_from_file_linear<P: AsRef<std::path::Path>>(
    device: &ash::Device,
    allocator: &Arc<Mutex<Allocator>>,
    queue: vk::Queue,
    command_pool: vk::CommandPool,
...
fn load_texture_from_file_with_format<P: AsRef<std::path::Path>>(
    device: &ash::Device,
    allocator: &Arc<Mutex<Allocator>>,
    queue: vk::Queue,
    command_pool: vk::CommandPool,
