...
impl<'a> Ui<'a> {
    ...
    pub fn shift_held(&self) -> bool {
        self.input.is_key_held(ImKey::ShiftLeft) || self.input.is_key_held(ImKey::ShiftRight)
    }

    /// `true` if either Ctrl is held this frame.
    pub fn ctrl_held(&self) -> bool {
        self.input.is_key_held(ImKey::ControlLeft) || self.input.is_key_held(ImKey::ControlRight)
    }
        ...
        F: FnOnce() -> T,
    {
        ...
        if hovered && self.input.left_just_pressed() && self.state.drag.is_none() {
            self.state.drag = Some(DragState::new(id, rect, mp, make_payload()));
        }
    ...
    pub fn take_drop<T: 'static + Send + Sync>(&mut self, rect: Rect) -> Option<DroppedPayload<T>> {
        if !self.input.left_just_released() {
            return None;
        }
    ...
    pub fn drag_active(&self) -> bool {
        self.state.drag.as_ref().map(|d| d.active).unwrap_or(false)
    }
    ...
    /// Returns the active drag's *grab offset* as a fraction
    /// `(fx, fy)` of the source widget's size — i.e. where
    /// inside the source rect the press happened, normalised
    /// to `[0, 1]`. `None` if no drag is active. Used by ghost
    /// rendering and grid-snap previews so the cursor stays at
    /// the same relative point inside the dragged item.
    pub fn drag_grab_frac(&self) -> Option<(f32, f32)> {
        let drag = self.state.drag.as_ref()?;
        if !drag.active {
            return None;
        }
        let w = drag.source_rect.width().max(1.0);
        let h = drag.source_rect.height().max(1.0);
        let fx = ((drag.press_pos.x - drag.source_rect.min.x) / w).clamp(0.0, 1.0);
        let fy = ((drag.press_pos.y - drag.source_rect.min.y) / h).clamp(0.0, 1.0);
        Some((fx, fy))
    }

    /// Like [`Self::take_drop`] but resolves containment using
    /// an explicit `point` instead of the cursor. Useful for
    /// grid-snapped drops where the destination cell is
    /// computed from the ghost's anchor rather than the raw
    /// mouse position.
    pub fn take_drop_at<T: 'static + Send + Sync>(
        &mut self,
        rect: Rect,
        point: Pos2,
    ...
    pub fn take_drop_outside<T: 'static + Send + Sync>(&mut self) -> Option<DroppedPayload<T>> {
        if !self.input.left_just_released() {
            return None;
        }
        let drag = self.state.drag.as_ref()?;
    ...
    pub fn draw_rect(&mut self, rect: Rect, color: Color) {
        self.layers
            .push(self.current_layer, DrawCmd::Rect { rect, color });
    }
    ...
    /// Vertical-gradient filled rect. `top` colour at the upper
    /// edge, `bot` at the lower. The overlay shader interpolates
    /// per-vertex colours across the quad, so this is the same
    /// primitive cost as `draw_rect` — no extra pipeline.
    pub fn draw_gradient_rect(&mut self, rect: Rect, top: Color, bot: Color) {
        self.layers
            .push(self.current_layer, DrawCmd::GradRect { rect, top, bot });
    }

    /// Vertical-gradient filled rounded rect.
    pub fn draw_rounded_gradient_rect(&mut self, rect: Rect, radius: f32, top: Color, bot: Color) {
        if radius <= 0.0 {
            self.draw_gradient_rect(rect, top, bot);
        } else {
            self.layers.push(
                self.current_layer,
    ...
    /// Bilinear-gradient rect: one colour per corner. Lets a
    /// caller fade a band along the horizontal axis without
    /// stacking translucent rects (e.g. a top-bevel sheen
    /// that's bright in the middle and transparent at the
    /// horizontal edges).
    pub fn draw_grad4_rect(&mut self, rect: Rect, tl: Color, tr: Color, bl: Color, br: Color) {
        self.layers.push(
            self.current_layer,
            DrawCmd::Grad4Rect {
                rect,
    ...
    /// colour at the geometric centre, `edge` along the
    /// bounding ellipse with smoothstep falloff. Used for the
    /// soft "polished blood-red" hotspot on action buttons.
    pub fn draw_rounded_radial_rect(
        &mut self,
        rect: Rect,
        radius: f32,
        edge: Color,
    ...
    /// metal rather than a smooth gradient. Costs the same as
    /// the smooth version (one tessellation pass).
    pub fn draw_rounded_radial_rect_noisy(
        &mut self,
        rect: Rect,
        radius: f32,
        edge: Color,
    ...
    /// Rounded outline of constant pixel thickness. Composed
    /// of a real triangle strip around the perimeter (the
    /// curved corners are explicit arcs), so the outline
    /// wraps the rounded corners cleanly even when there's
    /// no fill behind it. Falls back to `draw_outline` for
    /// `radius <= 0`.
    pub fn draw_rounded_outline(&mut self, rect: Rect, radius: f32, thickness: f32, color: Color) {
        if thickness <= 0.0 {
            return;
        }
        if radius <= 0.0 {
    ...
    pub fn draw_outline(&mut self, rect: Rect, thickness: f32, color: Color) {
        ...
        // Left
        self.draw_rect(
            Rect::from_xywh(rect.x(), rect.y() + t, t, rect.height() - 2.0 * t),
            color,
        );
        // Right
        self.draw_rect(
