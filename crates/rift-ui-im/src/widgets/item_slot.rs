...
pub struct ItemSlot<'a> {
    ...
    anchored: bool,
    /// When set, suppress the slot background fill so the
    /// surface behind the slot (e.g. a stone-textured frame)
    /// shows through. Hover/selected highlights are still
    /// drawn as a translucent overlay.
    transparent_bg: bool,
    /// Visibility scalar in `[0, 1]`. `1.0` = fully opaque
    /// (default); values `< 1.0` paint a translucent dark
    /// wash over the slot to dim it (used by the stash
...
impl<'a> ItemSlot<'a> {
    ...
    /// Skip the opaque slot background fill. Useful when the
    /// slot sits on a textured panel and you want only the
    /// outline + icon to render. Hover/selected states still
    /// draw a translucent highlight.
    pub fn transparent_bg(mut self, on: bool) -> Self {
        self.transparent_bg = on;
        self
    }

    /// Visibility multiplier in `[0, 1]`. Values `< 1.0`
    /// paint a dark wash over the slot after everything else
    /// has rendered, fading the icon + tint without hiding
    /// it. Used by the stash filter row.
    pub fn dim_alpha(mut self, a: f32) -> Self {
        self.dim_alpha = a.clamp(0.0, 1.0);
        self
    }
        ...
        T: 'static + Send + Sync,
    {
        ...
        let right_clicked = self.enabled && hovered && ui.input().right_clicked();
    ...
    pub fn show_ghost(self, ui: &mut Ui<'_>) {
        ...
        self.show_ghost_size(ui, s, s);
    }

    /// Like [`Self::show_ghost`] but with an explicit pixel
    /// `(width, height)`. Used by multi-cell inventory items
    /// so the ghost matches the item's actual footprint
    /// (`cell_w * cell_px` by `cell_h * cell_px`) instead of a
    /// generic square. Honours the active drag's *grab offset*
    /// — the cursor stays at the same relative point inside
    /// the ghost that the user originally clicked, so picking
    /// up a 2×3 item by its top-left no longer makes the ghost
    /// jump to centre on the cursor.
    pub fn show_ghost_size(self, ui: &mut Ui<'_>, w: f32, h: f32) {
        let mp = ui.mouse_pos();
        let (fx, fy) = ui.drag_grab_frac().unwrap_or((0.5, 0.5));
        let rect = Rect::from_xywh(mp.x - fx * w, mp.y - fy * h, w, h);
    ...
    fn render_into(&self, ui: &mut Ui<'_>, rect: Rect, hovered: bool) {
        ...
        // outlines + icon all read as faded together. We
        // never let the wash fully blot the slot out so the
...
pub struct SlotInteraction<T> {
    ...
    pub clicked: bool,
    /// True iff the slot was right-clicked this frame.
    /// Consumes the right-click flag for the frame, so only
    /// the actually-hovered slot will report `true`.
