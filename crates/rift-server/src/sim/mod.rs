...
impl Sim {
    ...
    fn spawn_legendary_test_items(&mut self) {
        ...
        // TEMP: drop a Studded Vest so the modular equipment
        // visual pipeline (BaseItem::model_path → attached
        // SkinnedMesh) has something to pick up + equip in the
        // hub. Remove once a vendor / loot path covers it.
        if let Some(base) = find_base("light_chest") {
            let mut rng = rift_game::loot::LootRng::new(0xC0FFEE_C4E57);
            let item = rift_game::loot::Item::roll(
                base,
                rift_game::loot::Rarity::Common,
                1,
                &mut rng,
            );
            self.spawn_dropped_loot(item, spawn + glam::Vec3::new(0.0, 0.0, 1.5));
        }
    }
